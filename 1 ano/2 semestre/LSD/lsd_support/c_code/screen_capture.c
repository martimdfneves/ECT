...
//
// Special program to capture a VGA screen (produces a ASCII ppm file with P3 header)
//
...
e:  close(sc->fd);
  ...
  //
  sc->current_tio.c_iflag = IGNBRK | IGNPAR; // ignore parity and framing errors
  if(sc->parity != 'N')
...
{
  fprintf(fp,"\n");
  fprintf(fp,"VGA screen capture (LSD.TOS, April 2018)\n");
...
{
  banner(stderr);
  fprintf(stderr,"Usage: lsd_term [-d device_name] [com_spec]\n");
  fprintf(stderr,"\n");
  ...
  fprintf(stderr,"\n");
  fprintf(stderr,"the default com_spec is 230400,N,8,1\n");
...
int main(int argc,char **argv)
{
  ...
  fp = fopen("screen_capture.ppm","wb");
  if(fp == NULL)
    return 1;
  ...
  printf("stop bits ..... %d\n",sc.stop_bits);
  printf("\n");
  printf("--- to start a screen capture press the space key ---\n");
  printf("\e[0m");
  fflush(stdout);
  if(snprintf(title,sizeof(title),"VGA screen capture %d,%c,%d,%d on %s",sc.baud_rate,sc.parity,sc.data_bits,sc.stop_bits,sc.device_name) >= sizeof(title))
  ...
  for(;must_exit == 0;)
  {
    ...
    if(select(1 + (fd > sc.fd ? fd : sc.fd),&read_mask,NULL,NULL,&time_out) > 0)
    {
      ...
      if(FD_ISSET(fd,&read_mask) && read(fd,&c,(size_t)1) == (ssize_t)1 && c == ' ' && w < 0)
      { // terminal -> FPGA (start screen capture)
        ...
        printf("Screen capture in progress\n");
        w = h = -1;
        p = 0;
        b3 = b2 = b1 = b0 = -1;
      }
  ...
  printf("\nGoodbye!\n");
  set_terminal_title("");
  if(w < 0 || h < 0 || b3 != -1 || b2 != -1 || b1 != -1 || b0 != -1 || p != w * h)
    printf("Something is wrong [w=%d,h=%d,b3=%d,b2=%d,b1=%d,b0=%d,p=%d]\n",w,h,b3,b2,b1,b0,p);
  else
    printf("%dx%d screen image captured\n",w,h);
  fclose(fp);
  return 0;
}
