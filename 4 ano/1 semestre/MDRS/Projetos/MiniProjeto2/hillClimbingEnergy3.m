function [sol, load, Loads, energy] = hillClimbingEnergy3(nNodes, Links, T, sP, nSP, sol, load, Loads, energy, L)
    nFlows= size(T,1);
        ...
        for flow = 1 : nFlows
            ...
            for path = 1 : nSP(flow)
                if path ~= sol(flow)
                    ...
                    auxSol(flow) = path;
                    % calculate loads
                    [auxLoad, auxLoads, auxEnergy] = calculateLinkEnergy3(nNodes,Links,T,sP,auxSol, L);
