function [load, Loads, energy] = calculateLinkEnergy(nNodes,Links,T,sP,Solution, L)
    nFlows= size(T,1);
    nLinks= size(Links,1);
    aux= zeros(nNodes);
    for i= 1:nFlows
    ...
    if load > 50   % If the worst link load is greater than max capacity , energy will be infinite 
        energy = inf;
    else
        energy = 0;
        for i= 1:nLinks
