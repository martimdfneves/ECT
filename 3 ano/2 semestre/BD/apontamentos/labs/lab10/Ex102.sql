...
-- a)
CREATE CLUSTERED INDEX rididx ON aula10.mytemp(rid);
...
SET nocount ON;
SET IDENTITY_INSERT AULASBD.aula10.mytemp ON; 

WHILE @val <= @nelem
BEGIN
	DBCC DROPCLEANBUFFERS;

	INSERT aula10.mytemp(rid,at1,at2,at3,lixo)
...
PRINT 'Inserted ' + str(@val) +' total elements!'

SET @end_time = GETDATE();
PRINT 'Total ms: ' + CONVERT(VARCHAR(20), 
...
-- Total ms: 110374
SELECT * FROM sys.dm_db_index_physical_stats(db_id('AULASBD'), 
...
-- c)
ALTER INDEX ALL ON aula10.mytemp REBUILD WITH (FILLFACTOR=65);
-- run insertion here
-- Insertion times:
-- Fillfactor = 65 -> Total ms: 142106

ALTER INDEX ALL ON aula10.mytemp REBUILD WITH (FILLFACTOR=80);
-- run insertion here
-- Insertion times:
-- Fillfactor = 80 -> Total ms: 165840

ALTER INDEX ALL ON aula10.mytemp REBUILD WITH (FILLFACTOR=90);
-- run insertion here
-- Insertion times:
...
-- operação é pesada em termos computacionais.

-- O que observamos é que o fill factor maior cria maiores tempos comprovando
...
BEGIN
	...
	INSERT aula10.mytemp(at1,at2,at3,lixo)
...
-- Como mencionado em cima, o facto de termos page splits é penalizador a 
...
-- No indexes
DROP INDEX IF EXISTS rididx on aula10.mytemp;
-- run insert here
-- time -> Total ms: 108220

-- All indexes
CREATE INDEX rididx on aula10.mytemp(rid);
CREATE INDEX at1idx on aula10.mytemp(at1);
CREATE INDEX at2idx on aula10.mytemp(at2);
CREATE INDEX at3idx on aula10.mytemp(at3);
CREATE INDEX lixoidx on aula10.mytemp(lixo);
-- run insert here
...
-- Isto deve-se principalmente pelo facto de, para alem da inserção dos dados,
-- o sgbd executa tmb instruções em relação aos indices (p ex escrever 
-- informação relativa aos indices em paginas)
