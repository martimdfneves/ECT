...
CREATE FUNCTION Projeto.PurchasedProductsPerClient(@NIF BIGINT) RETURNS @PurchasedProducts TABLE
												  (Number INT, Product_Name VARCHAR(40), N�Units INT, Date DATE, Purchase_Price DECIMAL(5,2))
AS
BEGIN
	INSERT
@PurchasedProducts
SELECT Compra.NumCompra             AS Number,
       Artigo.Nome                  AS Product_Name,
       Artigo_Comprado.QuantArtigos AS N�Units, Compra.Data AS Date, Compra.Montante AS Purchase_Price
...
--Test Function
SELECT *
FROM Projeto.PurchasedProductsPerClient(123456789);
--------------------------------------------------------------------------------------------
GO
CREATE FUNCTION Projeto.ReturnedProductsPerClient(@NIF BIGINT) RETURNS @ReturnedProducts TABLE
												  (Number INT, Product_Name VARCHAR(40), N�Units INT, Date DATE, Returned_Value DECIMAL(5,2))
AS
BEGIN
	INSERT
@ReturnedProducts
SELECT Devolucao.IDDevolucao         AS Number,
       Artigo.Nome                   AS Product_Name,
       Artigo_Devolvido.QuantArtigos AS N�Units, Devolucao.Data AS Date, Devolucao.Montante AS Returned_Value
...
--Test Function
SELECT *
FROM Projeto.ReturnedProductsPerClient(123456789);
--------------------------------------------------------------------------------------------
GO
...
BEGIN
	INSERT
@Sales
SELECT Compra.NumCompra AS Sale_Number, Compra.Data AS Date, Compra.Montante AS Sale_Price, Compra.NIF AS NIF
...
--Test Function
SELECT *
FROM Projeto.SoldProductsPerEmployee(112034);
---------------------------------------------------------------------------------------------
GO
...
BEGIN
	INSERT
@Returns
SELECT Devolucao.IDDevolucao AS Return_Number, Devolucao.Data AS Date, Devolucao.Montante AS Returned_Value, Devolucao.NIF AS NIF
...
--Test Function
SELECT *
FROM Projeto.ReturnedProductsPerEmployee(112034);
---------------------------------------------------------------------------------------------
GO
...
SELECT @Flag = COUNT(*)
FROM Projeto.Artigo
WHERE Artigo.Codigo = @Code
  AND Artigo.Preco = @Price
  AND Artigo.Nome = @Name
  AND Artigo.Categoria = @Type;
IF
(@Flag = 1)
BEGIN
RETURN 1;
...
GO
--Test Function
SELECT Projeto.CheckNewProdut(156428, 69.99, 'Camisola Equipamento SLB', 'Vestu�rio');